...
class PayrollController extends AccountBaseController
{
    ...
    public function show($id)
    {
        ...
        $this->depositRefund = $this->salarySlip->deposit_refund;
        ...
        $this->incomeTax = $this->salarySlip->income_tax;
    ...
    public function update(Request $request, $id)
    {
        ...
        $depositRefund = $request->deposit_refund;
        ...
        $loan = $request->loan;
        $ssb = $request->ssb;
        $incomeTax = $request->income_tax;

        $basicSalaryInMonth = $salarySlip->monthly_salary + $technicalAllowance + $livingCostAllowance + $specialAllowance + $otherAllowance + $depositRefund;
        $totalDetection = $salarySlip->absent + $salarySlip->leave_without_pay_detection + $otherDetection + $creditSales + $deposit + $loan + $ssb + $incomeTax;
        ...
        $salarySlip->deposit_refund = $depositRefund;
        ...
        $salarySlip->ssb = $ssb;
        $salarySlip->income_tax = $incomeTax;
        ...
        $userAllowance->deposit_refund = $depositRefund;
        ...
        $userDetection->income_tax = $incomeTax;
    ...
    public function generatePaySlip(Request $request)
    {
        ...
        foreach ($users as $user) {
            ...
            if ($endDate->greaterThan($joiningDate)) {
                ...
                $depositRefund = $allowance ? $allowance->deposit_refund : 0;
                ...
                $depositRefund = $perDayDepositRefund * $daysInMonth;
                ...
                $incomeTax = $detuction ? $detuction->income_tax : 0;

                // detection calculation
                $totalDetection = ($totalLeaveWithoutPaySalary) + $otherDetection + $creditSales + $deposit + $loan + $ssb + $incomeTax + $absentDetection;
    ...
    public function countAttendace($startDate, $endDate, $userId, $daysInMonth, $useAttendance, $joiningDate, $exitDate)
    {
        if ($useAttendance) {
            ...
            $holidayDates = $this->getHolidayByDates($HolidayStartDate->toDateString(), $HolidayEndDate->toDateString(), $userId)->pluck('holiday_date')->values(); // Getting Holiday Data
            ...
            $query = User::query()
                ...
                ->addSelect([
                    'basic_salary' => \App\Models\Allowance::select('basic_salary')
                        ->whereColumn('user_id', 'users.id')
                        ->limit(1),

                    'attendance_count' => \App\Models\Attendance::select(DB::raw('COUNT(DISTINCT DATE(clock_in_time))'))
                        ->whereColumn('user_id', 'users.id')
                        ->whereBetween(DB::raw('DATE(clock_in_time)'), [$startDate, $endDate])
                        ->whereNotIn(DB::raw('DATE(clock_in_time)'), $holidayDates)
                        // Exclude Off-days (employee_shift_id = 1)
                        ->whereNotExists(function ($query) {
                            $query->select(DB::raw(1))
                                ->from('employee_shift_schedules as ess')
                                ->whereColumn('ess.user_id', 'attendances.user_id')
                                ->whereColumn(DB::raw('DATE(ess.shift_start_time)'), DB::raw('DATE(attendances.clock_in_time)'))
                                ->where('ess.employee_shift_id', 1);
                        })
                        // Exclude Approved Leaves
                        ->whereNotExists(function ($query) {
                            $query->select(DB::raw(1))
                                ->from('leaves as l')
                                ->whereColumn('l.user_id', 'attendances.user_id')
                                ->whereColumn(DB::raw('DATE(l.leave_date)'), DB::raw('DATE(attendances.clock_in_time)'))
                                ->where('l.status', 'approved');
                        }),

                    'offday_count' => \App\Models\EmployeeShiftSchedule::select(DB::raw('COUNT(DISTINCT DATE(shift_start_time))'))
                        ->whereColumn('user_id', 'users.id')
                        ->whereBetween(DB::raw('DATE(shift_start_time)'), [$startDate, $endDate])
                        ->where('employee_shift_id', 1)
                        ->whereNotIn(DB::raw('DATE(shift_start_time)'), $holidayDates),

                    'leave_count' => \App\Models\Leave::select(DB::raw('COUNT(DISTINCT DATE(leave_date))'))
                        ->whereColumn('user_id', 'users.id')
                        ->whereBetween(DB::raw('DATE(leave_date)'), [$startDate, $endDate])
                        ->where('status', 'approved')
                        ->whereNotIn(DB::raw('DATE(leave_date)'), $holidayDates)
                        // Exclude if it's also an off-day
                        ->whereNotExists(function ($query) {
                            $query->select(DB::raw(1))
                                ->from('employee_shift_schedules as ess')
                                ->whereColumn('ess.user_id', 'leaves.user_id')
                                ->whereColumn(DB::raw('DATE(ess.shift_start_time)'), DB::raw('DATE(leaves.leave_date)'))
                                ->where('ess.employee_shift_id', 1);
                        }),
                ]);

            $results = $query->get()->map(function ($user) use ($holidayCount, $totalDays) {
                $att = $user->attendance_count ?? 0;
                $off = $user->offday_count ?? 0;
                $lve = $user->leave_count ?? 0;

                $user->holiday_count = $holidayCount;
                $user->total_payable_day_in_month = $totalDays;
                $user->pay_days = $att + $off + $lve + $holidayCount;

                $user->per_day_salary = $user->basic_salary > 0
                    ? round($user->basic_salary / $totalDays, 0)
                    : 0;

                $user->basic_salary_in_month = round($user->pay_days * $user->per_day_salary, 0);

                return $user;
            })
                ->whereIn('id', $userId);

            $payDays = $results->first()->pay_days ?? 0;
