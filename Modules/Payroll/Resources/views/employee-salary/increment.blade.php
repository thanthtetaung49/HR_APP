...
<div class="modal-body">
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="form-group">
                <x-forms.label :fieldLabel="__('payroll::modules.payroll.basicSalary')" fieldId="netSalary" />
                <h6>{{ currency_format($allowance->basic_salary, $currency->currency ? $currency->currency->id : company()->currency->id) }}
                </h6>
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                <x-forms.label :fieldLabel="__('payroll::modules.payroll.technicalAllowance')" fieldId="netSalary" />
                <h6>{{ currency_format($allowance->technical_allowance, $currency->currency ? $currency->currency->id : company()->currency->id) }}
                </h6>
            </div>
        </div>
    </div>
    <x-form id="updateSalary" method="POST">
        ...
        <div class="form-body">
            <div class="row">
                <div class="col-lg-4">
                    <x-forms.select fieldId="type" :fieldLabel="__('payroll::modules.payroll.valueType')" fieldName="type" fieldRequired="true">
                ...
                <div class="col-lg-4">
                    <x-forms.number fieldId="amount" class="annualIncrementAmount" :fieldLabel="__('payroll::modules.payroll.increaseAmount')"
                        fieldName="amount" fieldRequired="true">
                    </x-forms.number>
                </div>

                <div class="col-lg-4">
                    <x-forms.datepicker fieldId="date" class="incrementDate" :fieldLabel="__('payroll::modules.payroll.incrementDate')" fieldName="date"
                        :fieldPlaceholder="__('placeholders.date')" :fieldValue="now($company->timezone)->format($company->date_format)" />
                </div>
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="form-group currentMonthlySalary">
                <x-forms.label :fieldLabel="__('payroll::modules.payroll.livingCostAllowance')" fieldId="currentMonthlySalary" />
                <h6 id="grossMonthly">
                    {{ currency_format($allowance->living_cost_allowance, $currency->currency ? $currency->currency->id : company()->currency->id) }}
                </h6>
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                <x-forms.label :fieldLabel="__('payroll::modules.payroll.specialAllowance')" fieldId="netSalary" />
                <h6 id="grossAnnual">
                    {{ currency_format($allowance->special_allowance, $currency->currency ? $currency->currency->id : company()->currency->id) }}
                </h6>
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#update-salary').click(function() {
            console.log($('#updateSalary').serialize());
        ...
        $("#type").on("change", function(e) {
            ...
            var incrementDate = '';
            if (type == 'increment') {
                typeName = "{{ __('payroll::modules.payroll.increaseAmount') }}"
                incrementDate = "{{ __('payroll::modules.payroll.incrementDate') }}"
                incrementCurrentSalary =
                    "{{ __('payroll::modules.payroll.incrementMonthlyGrossSalary') }}"
            } else {
                typeName = "{{ __('payroll::modules.payroll.decreaseAmount') }}"
                incrementDate = "{{ __('payroll::modules.payroll.decrementDate') }}"
                incrementCurrentSalary =
                    "{{ __('payroll::modules.payroll.decrementMonthlyGrossSalary') }}"
            }
