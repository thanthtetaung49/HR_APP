            ...
            <x-form action="{{ route('criteria.update', $criteria->id) }}" method="PUT">
                ...
                <div class="add-client bg-white rounded">
                    <h4 class="mb-0 p-20 f-21 font-weight-normal  border-bottom-grey">
                        @lang('app.menu.editExitReason')</h4>
                    <div class="row p-20">
                        @if (isset($fields) && count($fields) > 0)
                            @foreach ($fields as $field)
                                <div class="col-md-6">
                                    <x-forms.label class="my-3" fieldId="exit_reason_id" :fieldLabel="$field->label"
                                    ...
                                    <x-forms.input-group>
                                        <select class="form-control select-picker mt" name="exit_reason_id"
                                            ...
                                            @foreach ($field->values as $key => $value)
                                                <option value="{{ $key }}"
                                                    @if ($key == $criteria->exit_reason_id) selected @endif>{{ $value }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </x-forms.input-group>
