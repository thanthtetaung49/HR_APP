...
<div class="modal-body bg-grey">
                    ...
                    <div class="col-12">
                        <h4 class="card-title f-15 f-w-500 text-darkest-grey mb-0">
                            <a href="{{ route('employees.show', [$attendance->user->id]) }}"
                                class="text-darkest-grey">{{ $attendance->user->name }} @if (user() && user()->id == $attendance->user->id)
    ...
    <div class="row">
        <div class="col-md-6">
            <x-cards.data :title="__('app.date') .
                ' - ' .
                $attendanceDate->translatedFormat(company()->date_format) .
                ' (' .
        ...
        <div class="col-md-6">
            ...
            <x-cards.data :title="__('modules.employees.activity')">
                @if ($addAttendancePermission == 'all' && $maxClockIn)
                    <x-slot name="action">
                        <a class="btn-primary rounded f-12 py-1 px-2" href="javascript:;"
                            onclick="addAttendance({{ $attendance->user->id }})"
                            data-attendance-id="{{ $attendance->user->id }}">@lang('app.add')</a>
                    </x-slot>
                @endif
                                        ...
                                        @if (!is_null($item->employee_shift_id))
                                            @if ($item->shift->shift_name != 'Day Off')
                                                <span class="badge badge-info ml-2"
                                                    style="background-color: {{ $item->shift->color }}">{{ $item->shift->shift_name }}</span>
                                            @else
                                                <span
                                    ...
                                    <p class="res-activity-time">
                                        ...
                                        @if ($item->work_from_type != '')
                                            @if ($item->work_from_type == 'other')
                                                ...
                                                {{ $item->location }}
                                                {{ $item->working_from != '' ? '(' . $item->working_from . ')' : '' }}
                                            @else
                                                <i class="fa fa-map-marker-alt ml-2"></i>
                                        ...
                                        @if ($item->latitude != '' && $item->longitude != '')
                                            <a href="https://www.google.com/maps/search/?api=1&query={{ $item->latitude }}%2C{{ $item->longitude }}"
                                                target="_blank">
                                ...
                                <div class="dropdown ml-auto comment-action">
                                    @if (
                                        $editAttendancePermission == 'all' ||
                                            ...
                                            ($editAttendancePermission == 'both' &&
                                                ($item->added_by == user()->id || $attendance->user->id == user()->id)) ||
                                            ...
                                            ($deleteAttendancePermission == 'both' &&
                                                ($item->added_by == user()->id || $attendance->user->id == user()->id)))
                                        <button class="btn btn-lg f-14 py-0 text-lightest  rounded  dropdown-toggle"
